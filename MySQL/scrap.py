...
def get_images(response):
    for item in response:
        if item[2] == 'Movie':
            url = f"{root}{item[1].replace(' ', '%20').replace('&', 'and')}%20Movie"
        else:
            url = f"{root}{item[1].replace(' ', '%20').replace('&', 'and')}%20Tv%20Show"
        print(f'\n\n{url}')
        r = s.get(url)
...
response = db.send_query("select * from titles where image is null")
get_images(response)